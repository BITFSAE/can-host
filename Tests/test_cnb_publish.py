...
class FallbackReleaseTest(unittest.TestCase):
    ...
    def test_fallback_builds_expected_asset_urls_and_sizes(self) -> None:
        ...
        self.assertEqual([item["name"] for item in assets],
                         ["BITFSAE_CAN_Host_v0.9.0.zip", "BITFSAE_CAN_Host_v0.9.0.zip.sha256",
                          ...
                          "BITFSAE_CAN_Host_macOS_arm64_v0.9.0-update.zip",
                          "BITFSAE_CAN_Host_macOS_arm64_v0.9.0-update.zip.sha256"])
    ...
    def test_fallback_accepts_legacy_windows_only_release(self) -> None:
        def fake_probe(url, timeout=60.0):
            return 0 if "macOS_arm64" in url else 10

        with patch("cnb_publish.probe_remote_size", side_effect=fake_probe):
            _, assets = cnb_publish.fallback_release("BITFSAE/can-host", "v0.9.1")
        self.assertEqual(len(assets), 3)

    def test_fallback_accepts_legacy_dmg_pair_without_update_zip(self) -> None:
        def fake_probe(url, timeout=60.0):
            return 0 if "-update.zip" in url else 10

        with patch("cnb_publish.probe_remote_size", side_effect=fake_probe):
            _, assets = cnb_publish.fallback_release("BITFSAE/can-host", "v0.9.11")
    ...
    def test_fallback_rejects_partial_macos_release(self) -> None:
        ...
        with patch("cnb_publish.probe_remote_size", side_effect=fake_probe):
            with self.assertRaisesRegex(cnb_publish.CnbError, "macOS 安装与一键更新附件"):
